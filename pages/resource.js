...
export default function Resource() {
  ...
  // Fetch resource data from the public folder
  useEffect(() => {
    fetch('/stumble-higher-resources-final-1.json')
      .then(response => response.json())
      .then(data => {
        // Filter out header rows (e.g., rows with id "title" or starting with "-")
        const validResources = data.Books.filter(item => item.id !== "title" && !item.id.startsWith("-"))
  ...
  const loadRandomResource = (resourceList = resources) => {
    ...
    // Fallback timer: if the resource doesn’t load in 5 seconds, skip to the next resource.
  ...
  return (
    <div className={styles.container}>
      <div className={styles.main}>
        ...
        <iframe id="resource-iframe" className={styles.iframe} title="Resource Viewer"></iframe>
      </div>
