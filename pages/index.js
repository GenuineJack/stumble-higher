...
export default function Home() {
  ...
  useEffect(() => {
    ...
    function handleFrameMessages(event) {
      if (event.origin !== 'https://warpcast.com') return; // Ensure messages are from Warpcast

      const { type, payload } = event.data;

      switch (type) {
        case 'stumble':
          handleStumbleClick(); // Handle the "stumble" action
          break;
        default:
  ...
  function handleStumbleClick() {
    if (resources) {
      ...
      sendMessageToFrame('resourceChanged', { resourceTitle: randomResource?.Title });
    } else {
      fetchResources();
    }
  }
